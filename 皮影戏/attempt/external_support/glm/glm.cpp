...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'K':
			...
			case 'e':
				fscanf(file, "%f %f %f",
					&model->materials[nummaterials].emmissive[0],
					&model->materials[nummaterials].emmissive[1],
					&model->materials[nummaterials].emmissive[2]);
				break;
            default:
                /* eat up rest of line */
                fgets(buf, sizeof(buf), file);
		...
		case 'm':
			...
			case 'a':
				fscanf(file, "%s", buf);
				dir = glmDirName(model->pathname);
				model->materials[nummaterials].map_ka = (char*)malloc(sizeof(char) * (strlen(dir) + strlen(buf) + 1));
				strcpy(model->materials[nummaterials].map_ka, dir);
				strcat(model->materials[nummaterials].map_ka, buf);
				free(dir);
				break;
			case 'd':
				fscanf(file, "%s", buf);
				dir = glmDirName(model->pathname);
				model->materials[nummaterials].map_kd = (char*)malloc(sizeof(char) * (strlen(dir) + strlen(buf) + 1));
				strcpy(model->materials[nummaterials].map_kd, dir);
				strcat(model->materials[nummaterials].map_kd, buf);
				free(dir);
				break;
			default:
				/* eat up rest of line */
